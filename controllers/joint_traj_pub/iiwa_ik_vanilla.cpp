...
public:
        ...
        : Node("ik_angle_publisher")
    {
        ...
        error_iteration_publisher_ = this->create_publisher<std_msgs::msg::Float32MultiArray>("error_iteration_0", 10);
...
private:
    ...
    void publish_angles()
    {
...

        {
            ...
            error_iteration_msg.data.push_back(static_cast<float>(iterations));
    ...
    std::vector<double> calculate_ik(const std::vector<double> &target, const std::vector<double> &initial_angles)
    {
        ...
        for (size_t i = 0; i < 7; ++i)
        {
            double velocity = std::abs(q_dot[i] / 0.01); // Velocity for time step of 0.01s
        ...
        for (size_t i = 0; i < 7; ++i)
        {
            new_angles[i] = initial_angles[i] + scaling_factor * q_dot[i]; // Starts to oscillate if alpha > 0.8
        }
